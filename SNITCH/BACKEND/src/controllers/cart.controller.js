...
import { getCartDetils } from "../dao/cart.dao.js";
...
export async function getCart(req, res) {
  ...
  let cart = await getCartDetils(user._id);
...
export async function createOrderController(req, res) {
  ...
  const cart = await getCartDetils(user._id);
...
export async function verifyOrderController(req, res) {
  ...
  const isPaymentValid = validatePaymentVerification(
    ...
    razorpay_signature,
    CONFIG.RAZORPAY_TEST_SECRET_KEY,
  );
  ...
  if (!isPaymentValid) {
    ...
    return res.status(400).json({
      message: "Payment verifycation faild",
      success: false,
    });
  }
  ...
  return res.status(200).json({
    messaage: "Payment verify successfull",
    success: true,
  });
}
