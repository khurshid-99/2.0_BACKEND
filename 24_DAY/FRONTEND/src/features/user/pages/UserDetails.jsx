import { useUser } from "../hooks/useUser";
import "./userdetails.scss";

const UserDetails = () => {
  const { loding, userDetils, follow, unFollow, following, followers } =
    useUser();

  if (loding) {
    return (
      <main>
        <h1>Loding...</h1>
      </main>
    );
  }
  console.log(following);
  console.log(followers);
  return (
    <main className="userdetails_page">
      <div className="profile">
        <div className="left">
          <div className="laft_a">
            <div className="img_wraper">
              <img
                src="https://i.pinimg.com/736x/cf/ea/f9/cfeaf9d2ecd46be8d4ed2360718aa926.jpg"
                alt=""
              />
            </div>
            <h1>username</h1>
          </div>
          <div className="left_b">
            <p>bio</p>
          </div>
          <div className="follow_count">
            <h1>follower-1000</h1>
            <h1>following-2000</h1>
          </div>
        </div>
        <div className="right">
          <div className="follow">
            <h1>following</h1>
            <div className="follow_users">
              {following &&
                following.map((user) => (
                  <div className="inner_follow_user">
                    <div className="profile_wraper">
                      <img
                        src="https://i.pinimg.com/736x/cf/ea/f9/cfeaf9d2ecd46be8d4ed2360718aa926.jpg"
                        alt=""
                      />
                    </div>
                    <h1>{user.followee}</h1>
                  </div>
                ))}
            </div>
          </div>
          <div className="follow">
            <h1>follower</h1>
            <div className="follow_users">
              {followers &&
                followers.map((user) => (
                  <div className="inner_follow_user">
                    <div className="profile_wraper">
                      <img
                        src="https://i.pinimg.com/736x/cf/ea/f9/cfeaf9d2ecd46be8d4ed2360718aa926.jpg"
                        alt=""
                      />
                    </div>
                    <h1>{user.follower}</h1>
                  </div>
                ))}
            </div>
          </div>
          <div className="follow">
            <h1>others</h1>
            <div className="follow_users">
              <div className="inner_follow_user">
                <div className="profile_wraper">
                  <img
                    src="https://i.pinimg.com/736x/cf/ea/f9/cfeaf9d2ecd46be8d4ed2360718aa926.jpg"
                    alt=""
                  />
                </div>
                <h1>username</h1>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="posts">
        <div className="post">
          <img
            src="https://i.pinimg.com/736x/cf/ea/f9/cfeaf9d2ecd46be8d4ed2360718aa926.jpg"
            alt=""
          />
          <div className="buttons">
            <div className="left">
              <button
                onClick={() => {
                  // post.isLiked ? unLiked(post._id) : liked(post._id);:
                }}
              >
                {/* {"post.isLiked" ? ( */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="rgba(255,0,0,1)"
                >
                  <path fill="none" d="M0 0h24v24H0z"></path>
                  <path d="M12.001 4.52853C14.35 2.42 17.98 2.49 20.2426 4.75736C22.5053 7.02472 22.583 10.637 20.4786 12.993L11.9999 21.485L3.52138 12.993C1.41705 10.637 1.49571 7.01901 3.75736 4.75736C6.02157 2.49315 9.64519 2.41687 12.001 4.52853Z"></path>
                </svg>
                {/* ) : ( */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path fill="none" d="M0 0h24v24H0z"></path>
                  <path d="M12.001 4.52853C14.35 2.42 17.98 2.49 20.2426 4.75736C22.5053 7.02472 22.583 10.637 20.4786 12.993L11.9999 21.485L3.52138 12.993C1.41705 10.637 1.49571 7.01901 3.75736 4.75736C6.02157 2.49315 9.64519 2.41687 12.001 4.52853ZM18.827 6.1701C17.3279 4.66794 14.9076 4.60701 13.337 6.01687L12.0019 7.21524L10.6661 6.01781C9.09098 4.60597 6.67506 4.66808 5.17157 6.17157C3.68183 7.66131 3.60704 10.0473 4.97993 11.6232L11.9999 18.6543L19.0201 11.6232C20.3935 10.0467 20.319 7.66525 18.827 6.1701Z"></path>
                </svg>
                {/* )} */}
              </button>
              <button>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path fill="none" d="M0 0h24v24H0z"></path>
                  <path d="M5.76282 17H20V5H4V18.3851L5.76282 17ZM6.45455 19L2 22.5V4C2 3.44772 2.44772 3 3 3H21C21.5523 3 22 3.44772 22 4V18C22 18.5523 21.5523 19 21 19H6.45455Z"></path>
                </svg>
              </button>
              <button>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path fill="none" d="M0 0h24v24H0z"></path>
                  <path d="M21.7267 2.95694L16.2734 22.0432C16.1225 22.5716 15.7979 22.5956 15.5563 22.1126L11 13L1.9229 9.36919C1.41322 9.16532 1.41953 8.86022 1.95695 8.68108L21.0432 2.31901C21.5716 2.14285 21.8747 2.43866 21.7267 2.95694ZM19.0353 5.09647L6.81221 9.17085L12.4488 11.4255L15.4895 17.5068L19.0353 5.09647Z"></path>
                </svg>
              </button>
            </div>
            <button>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path fill="none" d="M0 0h24v24H0z"></path>
                <path d="M5 2H19C19.5523 2 20 2.44772 20 3V22.1433C20 22.4194 19.7761 22.6434 19.5 22.6434C19.4061 22.6434 19.314 22.6168 19.2344 22.5669L12 18.0313L4.76559 22.5669C4.53163 22.7136 4.22306 22.6429 4.07637 22.4089C4.02647 22.3293 4 22.2373 4 22.1433V3C4 2.44772 4.44772 2 5 2ZM18 4H6V19.4324L12 15.6707L18 19.4324V4Z"></path>
              </svg>
            </button>
          </div>
          <p>caption</p>
        </div>
        <div className="post">
          <img
            src="https://i.pinimg.com/736x/cf/ea/f9/cfeaf9d2ecd46be8d4ed2360718aa926.jpg"
            alt=""
          />
          <div className="buttons">
            <div className="left">
              <button
                onClick={() => {
                  // post.isLiked ? unLiked(post._id) : liked(post._id);:
                }}
              >
                {/* {"post.isLiked" ? ( */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="rgba(255,0,0,1)"
                >
                  <path fill="none" d="M0 0h24v24H0z"></path>
                  <path d="M12.001 4.52853C14.35 2.42 17.98 2.49 20.2426 4.75736C22.5053 7.02472 22.583 10.637 20.4786 12.993L11.9999 21.485L3.52138 12.993C1.41705 10.637 1.49571 7.01901 3.75736 4.75736C6.02157 2.49315 9.64519 2.41687 12.001 4.52853Z"></path>
                </svg>
                {/* ) : ( */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path fill="none" d="M0 0h24v24H0z"></path>
                  <path d="M12.001 4.52853C14.35 2.42 17.98 2.49 20.2426 4.75736C22.5053 7.02472 22.583 10.637 20.4786 12.993L11.9999 21.485L3.52138 12.993C1.41705 10.637 1.49571 7.01901 3.75736 4.75736C6.02157 2.49315 9.64519 2.41687 12.001 4.52853ZM18.827 6.1701C17.3279 4.66794 14.9076 4.60701 13.337 6.01687L12.0019 7.21524L10.6661 6.01781C9.09098 4.60597 6.67506 4.66808 5.17157 6.17157C3.68183 7.66131 3.60704 10.0473 4.97993 11.6232L11.9999 18.6543L19.0201 11.6232C20.3935 10.0467 20.319 7.66525 18.827 6.1701Z"></path>
                </svg>
                {/* )} */}
              </button>
              <button>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path fill="none" d="M0 0h24v24H0z"></path>
                  <path d="M5.76282 17H20V5H4V18.3851L5.76282 17ZM6.45455 19L2 22.5V4C2 3.44772 2.44772 3 3 3H21C21.5523 3 22 3.44772 22 4V18C22 18.5523 21.5523 19 21 19H6.45455Z"></path>
                </svg>
              </button>
              <button>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path fill="none" d="M0 0h24v24H0z"></path>
                  <path d="M21.7267 2.95694L16.2734 22.0432C16.1225 22.5716 15.7979 22.5956 15.5563 22.1126L11 13L1.9229 9.36919C1.41322 9.16532 1.41953 8.86022 1.95695 8.68108L21.0432 2.31901C21.5716 2.14285 21.8747 2.43866 21.7267 2.95694ZM19.0353 5.09647L6.81221 9.17085L12.4488 11.4255L15.4895 17.5068L19.0353 5.09647Z"></path>
                </svg>
              </button>
            </div>
            <button>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path fill="none" d="M0 0h24v24H0z"></path>
                <path d="M5 2H19C19.5523 2 20 2.44772 20 3V22.1433C20 22.4194 19.7761 22.6434 19.5 22.6434C19.4061 22.6434 19.314 22.6168 19.2344 22.5669L12 18.0313L4.76559 22.5669C4.53163 22.7136 4.22306 22.6429 4.07637 22.4089C4.02647 22.3293 4 22.2373 4 22.1433V3C4 2.44772 4.44772 2 5 2ZM18 4H6V19.4324L12 15.6707L18 19.4324V4Z"></path>
              </svg>
            </button>
          </div>
          <p>caption</p>
        </div>
      </div>
    </main>
  );
};

export default UserDetails;
